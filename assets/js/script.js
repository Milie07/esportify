		...
		form.addEventListener("submit", (e) => {
			...
			inputs.forEach((input) => {
				...
				if (input.id === "confirmPassword" && pwdInput && confirmPwdInput) {
					if (!validateConfirmationPassword(pwdInput, confirmPwdInput))
						isFormValid = false;
				}
			});
			if (!isFormValid) {
...
*/
	...
	function render(list) {
		...
		list.forEach((event) => {
			...
			if (dates)
				dates.textContent = `Du ${formatFR(event.startsAt)} au ${formatFR(
					event.endsAt
				)}`;
			if (players)
				players.textContent = `${event.playersRegistered}/${event.maxPlayers}`;
			if (detailsBtn) {
	...
	async function initEvents() {
		try {
			...
			if (organizerSelect) {
				const set = new Set(
					events
						.filter((e) => (e.status || "").toLowerCase() === "valide")
						.map((e) => e.organizer)
				);
				const orgs = Array.from(set).sort((a, b) =>
	...
	function gaugeInfo(playersRegistered, maxPlayers) {
		...
		return {
			count,
			label: `Reste ${remaining} place${remaining > 1 ? "s" : ""}`,
		};
	}
	...
	function fillModal(ev) {
		...
		elsModal.status.textContent =
			statusRaw === "valide" ? "Validé ✅" : ev.status || "—";
		...
		elsModal.btnJoin.classList.toggle(
			"d-none",
			!canShowJoin(ev.startsAt, ev.endsAt)
		);
...
document.addEventListener("DOMContentLoaded", () => {
	...
	function setConsent(value) {
		document.cookie =
			"esportify_consent=" + value + "; path=/; max-age=" + 180 * 24 * 60 * 60;
